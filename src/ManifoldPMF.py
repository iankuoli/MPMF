...
class ManifoldPMF:
    ...
    def coordinate_ascent(self, alpha, max_itr):
        ...
        while is_converge is False and i < max_itr:
            ...
            if self.epsilon > 0 or self.delta > 0:
                for k in range(self.K):
                    ...
                    self.matTheta_Shp[:, k] = self.epsilon * np.squeeze(np.asarray(self.matX.multiply(self.tensorPhi[k]).sum(1)))
                    self.matTheta_Shp[:, k] += self.delta * np.squeeze(np.asarray(self.matW.multiply(self.tensorRho[k]).sum(1)))

                self.matTheta_Shp += self.a

                self.matTheta_Rte = self.delta * (np.sum(self.matTheta * self.matPi[:, None], 0) * self.matPi[:, None]
                                                  - self.matTheta * (self.matPi * self.matPi)[:, None])
            ...
            if self.epsilon > 0 or self.mu > 0:
                ...
                self.matBeta_Rte = self.mu * (np.sum(self.matBeta * self.matGamma[:, None], 0) * self.matGamma[:, None]
                                              - self.matBeta * (self.matGamma * self.matGamma)[:, None])
        ...
        pickle.dump(self.matTheta, open("matTheta.p", 'wb'))
        pickle.dump(self.matBeta, open("matBeta.p", 'wb'))
        pickle.dump(self.matEpsilon, open("matEpsilon.p", 'wb'))
        pickle.dump(self.matEta, open("matEta.p", 'wb'))
        pickle.dump(self.matPi, open("matPi.p", 'wb'))
        pickle.dump(self.matGamma, open("matGamma.p", 'wb'))

    def stochastic_coordinate_ascent(self, alpha, max_itr):
        is_converge = False
        i = 0
        l = 0
        ...
        while is_converge is False and i < max_itr:
            ...
            if self.epsilon > 0 or self.mu > 0:
                for k in range(self.K):
                    self.matBeta_Shp[:, k] = self.epsilon * np.squeeze(
                        np.asarray(self.matX.multiply(self.tensorPhi[k]).sum(0)))
                    self.matBeta_Shp[:, k] += self.mu * np.squeeze(
                        np.asarray(self.matS.multiply(self.tensorSigma[k]).sum(1)))
